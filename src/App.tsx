...
import { Notice, Task, NewsletterAction, User, Child } from '@/types/index';
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // ロード中の状態を管琁E��るため�E新しいstate
  const [loadingAuth, setLoadingAuth] = useState(true);

  // Firebase Authのインスタンスを取征E
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (firebaseUser) => {
      if (firebaseUser) {
        // ログイン済みの場吁E
        // NOTE: ここでFirestoreからユーザーの追加プロフィール�E�Eirthdate, country, location�E�をロードする忁E��がありまぁE
        // 現状ではFirebase AuthのUserオブジェクトから直接取得できる惁E��のみを使用
        const currentUser: User = {
          uid: firebaseUser.uid,
          email: firebaseUser.email || '',
          username: firebaseUser.displayName || firebaseUser.email || '',
          birthdate: '', // Firestoreから取征E
          country: '',   // Firestoreから取征E
          location: '',  // Firestoreから取征E
        };
        setUser(currentUser);
      ...
      setLoadingAuth(false); // 認証状態�Eロードが完亁E
    });
  ...
  const signup = async (newUser: User, password: string) => {
    ...
    // onAuthStateChangedリスナ�Eが状態を更新するため、ここでは直接setIsAuthenticatedなどは呼び出さなぁE
  };
  ...
  const addNotice = (notice: Notice) => {
    ...
    const newTasks: Task[] = notice.extractJson.actions.map((action: NewsletterAction, index) => ({
      ...
      noticeId: notice.id,
      title: `${action.is_continuation ? '(継綁E ' : ''}${action.event_name}`,
...
const MainLayout = () => (
    <div className="min-h-screen bg-orange-50 text-gray-800">
      ...
      <main className="p-4 pb-24 sm:p-6 sm:pb-24 md:p-6 lg:p-8 max-w-4xl mx-auto">
        <Routes>
          <Route path="/dashboard" element={<DashboardPage />} /> {/* DashboardPageめEdashboardに移勁E*/}
...
// App.tsx の function App() 部刁E��以下�Eように変更
